        ...
        <div class="sidebar">
            ...
            <div class="main-content">
                ...
                <c:if test="${param.mensaje == 'exito'}">
                    <div class="alert alert-success text-center">Docente registrado con éxito.</div>
                </c:if>
                <c:if test="${param.mensaje == 'error'}">
                    <div class="alert alert-danger text-center">No se pudo registrar el docente.</div>
                </c:if>
                <c:if test="${param.mensaje == 'existe'}">
                    <div class="alert alert-warning text-center">Ya existe un docente con ese correo institucional.</div>
                </c:if>
                <c:if test="${param.mensaje == 'correo_invalido'}">
                    <div class="alert alert-warning text-center">El correo debe comenzar con 'C'.</div>
                </c:if>


                <div class="card p-4 mb-3">
                    <h3 class="card-title text-center mb-4"><i class="fas fa-chalkboard-teacher"></i> Registrar nuevo docente</h3>
                    <form action="${pageContext.request.contextPath}/RegistrarDocenteServlet" method="post" class="row g-3">
                        <div class="col-md-6">
                            <label>Nombre:</label>
                        ...
                        <div class="col-md-6">
                            <label>Correo institucional (empieza con C):</label>
                        ...
                        <div class="col-12 text-center">
                            <button type="submit" class="btn btn-success btn-form">Registrar Docente</button>
                ...
                <div class="card p-4 table-container">
                    <h4 class="card-title text-center mb-4"><i class="fas fa-list"></i> Docentes registrados</h4>
                            ...
                            <tbody>
                                <c:forEach var="docente" items="${docentes}">
                                    <tr>
                                        ...
                                        <td>
                                            <span class="password-field" id="password-${docente.id}">
                                                <c:forEach begin="1" end="${docente.password.length()}">*</c:forEach>
                                                </span>
                                                <span class="password-field d-none" id="password-real-${docente.id}">
                                                ${docente.password}
                                            </span>
                                        </td>
                                        <td>
                                            <button class="btn btn-info btn-sm btn-ver" onclick="togglePassword(${docente.id})"
                                                    id="btn-toggle-${docente.id}">Ver</button>
                                            <a href="EditarDocenteServlet?id=${docente.id}" class="btn btn-warning btn-sm">Editar</a>
                                            <c:choose>
                                                <c:when test="${docente.estado == 1}">
                                                    <a href="CambiarEstadoDocenteServlet?id=${docente.id}&estado=0"
                                                       class="btn btn-danger btn-sm"
                                                       onclick="return confirm('¿Seguro que deseas BLOQUEAR a este docente?');">
                                                        <i class="fas fa-user-slash"></i> Bloquear
                                                    </a>
                                                </c:when>
                                                <c:otherwise>
                                                    <a href="CambiarEstadoDocenteServlet?id=${docente.id}&estado=1"
                                                       class="btn btn-success btn-sm"
                                                       onclick="return confirm('¿Seguro que deseas ACTIVAR a este docente?');">
